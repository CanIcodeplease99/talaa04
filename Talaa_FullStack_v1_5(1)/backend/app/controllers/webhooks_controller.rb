class WebhooksController < ApplicationController
  ...
  def stripe
    ...
    begin
      ...
      event ||= JSON.parse(payload) # fallback (dev only)
    ...
    when 'payment_intent.succeeded'
      pi_id = data['id']
      settlement_ref = (data['metadata']||{})['settlement_ref']
      if fi = FundingIntent.find_by(provider_ref: pi_id)
        fi.update!(status:'succeeded')
        Metrics.inc(:funding_cleared)
      end
      if settlement_ref && (s = Settlement.find_by(transfer_id: settlement_ref))
        s.update!(funding_status:'cleared')
        Metrics.inc(:fx_trades_started)
